...
from .modules import LoRDepthWiseSepConv1d, LoRDepthWiseSepConv2d
...
class Wavegram_Logmel_Cnn14_Baseline(nn.Module):
    def __init__(
        self,
        sample_rate=32000,
        window_size=1024,
        ...
        super(Wavegram_Logmel_Cnn14_Baseline, self).__init__()
    ...
    def load_pretrained(self, path, device):
        ...
        pretrained_dict = torch.load(path, map_location=device)["model"]
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=0.5, training=self.training)
        global_embedding = torch.sigmoid(self.fc_audioset(x))

        return temporal_embedding, global_embedding


class Wavegram_Logmel_Cnn14_Adaptation(nn.Module):
    def __init__(
        self,
        sample_rate=32000,
        window_size=1024,
        ...
        super(Wavegram_Logmel_Cnn14_Adaptation, self).__init__()
